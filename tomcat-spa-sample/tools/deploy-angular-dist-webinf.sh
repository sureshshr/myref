...
SRC_DIR_DEFAULT="$ROOT_DIR/angular-web/dist/angular-web/browser"
...
if [[ ! -d "$SRC_DIR" ]]; then
  ...
  echo "  cd angular-web && npm install && npx ng build --configuration production --base-href /tomcat-spa-sample/app/" >&2
  exit 1
fi
